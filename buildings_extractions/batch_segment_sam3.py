...
densities = ['low','high']
sizes = [1024,512,256]
prompts = ['buildings','roofs', 'house']
input_bands = [1,2,3] # must be in orger of RGB

def run_sam3(size, density, prompts, input_bands):

    start_wall = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    start_perf = time.perf_counter()
    print(f"Script started at: {start_wall}")

    n = f"kigali_{density}_density_8_11_{size}_{prompt}"
    print(n)
    ...
    # 2. Setup your batch directories (Using clean forward slashes)
    input_folder = rf"D:\mwalter\world_bank\kigali_imagery\kigali_{density}_density_8_19_null_{size}"
    output_folder = f"D:/mwalter/sam/{n}_rgb/"
    ...
    for filename in os.listdir(input_folder):
        if filename.lower().endswith(('.tif', '.tiff')):
            ...
            try:
                sam3.set_image(image_path, bands=input_bands)
...
for density in densities:
    for prompt in prompts:
        for size in sizes:
            run_sam3(size, density, prompt, input_bands)
